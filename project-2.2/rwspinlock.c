...
void read_lock(void)
{
    ...
    do
    {
        do
        {
            expected = atomic_load(&rwlock);
        } while (expected & WRITER_BIT);

    } while (!atomic_compare_exchange_strong(&rwlock, &expected, expected + 1));
}
...
{
    int expected = 0;
    while (!atomic_compare_exchange_strong(&rwlock, &expected, WRITER_BIT))
    {
        expected = 0;
    }
}
...
{
    atomic_fetch_and(&rwlock, ~WRITER_BIT);
}
